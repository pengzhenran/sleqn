!==============================================================================
!  ★ 修正版 sleqn_dp.f90（推荐实际使用；sleqn.f90 为与文章代码逐行一致的原文版）
!
!  本文件与 sleqn.f90 算法完全相同，只修了两处：
!
!  【修正 1】把原文中"默认单精度"的字面常量提升为双精度（加 D0 / 改用 dsqrt、dfloat）。
!     受影响的有：1./sqrt(2.)、sqrt(1.+1./2./float(j))、sqrt(2.)、3./5.517、
!     6.371E3**2、6.371E8、pi=3.14159265、rhoave=5.517、0.298、0.604、5.97E27 等。
!     这些常量在原文里先按 REAL(4) 截断，带入约 1e-8 的相对误差；而 gdisc 中
!        temp(l) = (P_{l-1}(cosα) - P_{l+1}(cosα))/2
!     是两个≈1 的数相减（单元角半径 α 很小，差值只有 1e-6~1e-5 量级），
!     有效数字大量抵消，于是 1e-8 的输入误差被放大到约 1e-3 ——
!     导致单个网格单元的球谐系数出现千分之几的误差。
!
!  【修正 2】pmh 一行的分母。原文 float(2*l+1) 位于 Do l = 0, lsyn 循环之后，
!     此时 l = lsyn+1 = 181，分母变成 363；而极移项只在 l=2, m=1 处生效，
!     按物理意义分母应为 2*2+1 = 5。原写法把极移反馈削弱了约 72 倍。
!
!  验证（合成算例，100 个网格单元、niter=2，均与 sleqn.py 逐点比较）：
!     原文 sleqn.f90                 max|Δ| = 3.427e-03 cm  (占场最大值 0.33%)
!     仅修正 1（精度）                max|Δ| = 1.411e-03 cm  (0.14%)
!     修正 1 + 修正 2（本文件）        max|Δ| = 9.99e-15 cm   （机器精度）
!     其中"修正 1 对最终海平面场的影响" max|Δ| = 3.98e-03 cm (0.38%)，
!     "修正 2 的影响"                  max|Δ| = 1.41e-03 cm (0.14%)。
!     另外，gfortran -Wall -Wextra 编译本文件除一条 -Wdo-subscript 误报外无告警。
!
!  编译： gfortran -O2 -o sleqn_dp.exe sleqn_dp.f90
!         gfortran -O2 -static -o sleqn_dp.exe sleqn_dp.f90   （不依赖 MinGW 运行库，便于分发）
!  说明： 若不需要极移反馈，可直接注释掉 coefpm/pmp/pmh 三行及迭代中的极移分支，
!        此时修正 2 不再有影响。
!
!  【易用性增强，不影响任何计算】程序按"当前工作目录"读取输入文件。为避免双击运行时
!     因找不到文件而瞬间闪退，本文件在开头与 Filelist 循环中增加了 Inquire 文件存在性
!     检查，缺失时打印明确提示后停止。原文 sleqn.f90 中无此检查。
!==============================================================================
Program sleqn
!==============================================================================
...
!     输入文件检查用的变量（易用性增强，与算法无关）
      Logical lexist
      Character*60 missfile
...
!     网格与结果变量
 ...
 22   Format (F5.1, 1X, F5.1, 1X, E12.4)
      ...
      rhoave = 5.517D0
      arad = 6.371D8
      rkf = 0.00328475D0/0.00348118D0
      rk2b = 0.298D0
      h2b = 0.604D0
      lmax = lmost
!变量定义//

!//输入文件检查（易用性增强：双击运行时若文件不在当前目录，给出明确提示而不是闪退）
      missfile = ' '
      Inquire(File='land.fcn.1_deg', Exist=lexist)
      If(.Not. lexist) missfile = 'land.fcn.1_deg'
      Inquire(File='love_numbers', Exist=lexist)
      If(.Not. lexist .And. missfile==' ') missfile = 'love_numbers'
      Inquire(File='Filelist.txt', Exist=lexist)
      If(.Not. lexist .And. missfile==' ') missfile = 'Filelist.txt'
      If(missfile /= ' ') Then
        Print *, '============================================================'
        Print *, 'ERROR: required input file not found: ', Trim(missfile)
        Print *, 'This program reads ALL input files from the CURRENT DIRECTORY.'
        Print *, '  * If you double-clicked the exe, the current directory is'
        Print *, '    the folder that contains the exe.'
        Print *, '  * Put the exe together with land.fcn.1_deg, love_numbers,'
        Print *, '    Filelist.txt and your data files in ONE folder, then run.'
        Print *, '  * Or use the provided .bat launcher (it keeps the window open).'
        Print *, '============================================================'
        Stop
      EndIf
!输入文件检查//
...
!//赋值海面指代及love数
      Open (80, File='land.fcn.1_deg', Status='old')     ! ★1 陆地区域mask文件
...
!         write (99,*) rlon(i), rlat(j), ofcn(i, j)
      ...
      Open(90, File='love_numbers', Status='old')       ! ★2 love数文件名
      Read(90, 11)
      Read(90, 11)
      Do l = 0, llove
        Read(90, *) ldum, h(l), rk(l), rll
        coefh(l) = (1.+rk(l)-h(l))*3.D0*rho0/rhoave/float(2*l+1)
        coefp(l) = (1.+rk(l)-h(l))/(rk(l)+1)
      EndDo
!赋值海面指代及love数//

!//读入输入文件
      Open (100, File='Filelist.txt', Status='old')     ! ★3 控制文件
!     需要两列值，分别是输入文件名与输出结果文件名
 300  Continue
      Read(100, *, End=400) namein, nameout
!     检查 Filelist.txt 里列出的数据文件是否存在（相对路径按当前目录解析）
      Inquire(File=namein, Exist=lexist)
      If(.Not. lexist) Then
        Print *, '============================================================'
        Print *, 'ERROR: data file listed in Filelist.txt not found: ', Trim(namein)
        Print *, 'Paths written in Filelist.txt are relative to the CURRENT DIRECTORY.'
        Print *, '============================================================'
        Stop
      EndIf
      Open(10, File=namein, Status='old')
!     ★ 输出文件用 Status='replace'：原文未写 Status（默认 UNKNOWN），
!       gfortran 对"已存在的输出文件"会按只读方式打开，第二次运行即报
!       "Cannot write to file opened for READ" 而崩溃。用 'replace' 可覆盖旧结果。
      Open(20, File=nameout, Status='replace')
...
!     If you don't want to include that feedback, comment out these 2 lines.
      ...
      pmh = pmp*(1.+rk(2))*3.D0*rho0/rhoave/float(2*2+1)      ! ★ 原文为 float(2*l+1)，此时 l=181 → 分母 363，见头部说明
...
!//输入文件类型
      ...
      area1 = dlat*dlon*dtr*dtr*6.371D3**2
!     area1是不包括纬度因素的网格点面积
...
!     读取数据依次为网格点经度、纬度、水高增量(m)
      ...
      xlon = xlon*dtr
      xlat = xlat*dtr
      area = dlon*dtr*(dcos(x1*dtr)-dcos(x2*dtr))*6.371D3**2
!     area = area1*dsin(xlat)
      rmass = thick*100.D0*1.00D0*(area*1.D10)
      smass = smass + rmass
      ...
      Do j = 1, nth
        ...
        Do l = 0, lmax
          Do m = 0, l
            ...
            Else
              plm(l, m, j) = plmart(l, m)*dsqrt(2.D0)
            EndIf
          EndDo
        EndDo
      EndDo
...
!//输出文件
      zmass = hclm(0, 0)*arad**2*rho0*4.D0*pi/1.D15
...
!//使用Martin的递归关系计算归一化勒让德函数//
      ...
      Do m = 0, lmax
        ...
        If(m==0) Then
          ptemp(0) = 1.D0/dsqrt(2.D0)
        Else
          ptemp(0) = 1.0
          Do j = 1, m
            ptemp(0) = ptemp(0)*dsqrt(1.D0+1.D0/2.D0/dfloat(j))
          EndDo
          ptemp(0) = ptemp(0)/dsqrt(2.D0)
        EndIf

        If((lmax-m)>0) Then
          Do k = 1, lmax - m
            ptemp(k) = 2.D0*x*ptemp(k-1)*dsqrt(1.D0+(dfloat(m)-0.5D0)/dfloat(k)) &
                     *dsqrt(1.D0-(dfloat(m)-0.5D0)/dfloat(k+2*m))
            If(k>1) Then
              ptemp(k) = ptemp(k) - ptemp(k-2)*dsqrt(1.D0+4.D0/dfloat(2*k+2*m-3)) &
                       *dsqrt(1.D0-1.D0/dfloat(k))*dsqrt(1.D0-1.D0/dfloat(k+2*m))
            EndIf
          EndDo
        EndIf
...
!//计算均匀圆盘（单个网格单元）的斯托克斯系数//
      ...
      clat = 1.D0 - area/6.371D3**2/2.D0/pi
      alpha = dcos(clat)
!     alpha是角半径(以度为单位)   ← 原文如此，该变量其后并未使用
      ...
      Do l = 0, lmax1
        plmart(l, 0) = plmart(l, 0)*dsqrt(2.D0/dfloat(2*l+1))
      EndDo
      Do l = 1, lmax
      ...
      Do l = 0, lmax
        Do m = 0, l
          plmart(l, m) = plmart(l, m)*dsqrt(2.D0)
          If(m/=0) plmart(l, m) = plmart(l, m)*dsqrt(2.D0)
        EndDo
      EndDo
...
!     h是水高，单位为cm
      ...
      Do l = 0, lmax
        Do m = 0, l
          wc = temp(l)/dfloat(2*l+1)*plmart(l, m)*cos(m*rlon)
          ws = temp(l)/dfloat(2*l+1)*plmart(l, m)*sin(m*rlon)
          clm1(l, m) = clm1(l, m) + wc*3.D0/5.517D0*(1.D0+rk(l))/dfloat(2*l+1)*ha
          slm1(l, m) = slm1(l, m) + ws*3.D0/5.517D0*(1.D0+rk(l))/dfloat(2*l+1)*ha
        EndDo
      EndDo
